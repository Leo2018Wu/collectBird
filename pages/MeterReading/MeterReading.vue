		...
		<evan-form class="evanForm" :hide-required-asterisk="hideRequiredAsterisk" ref="form" :model="info">
			...
			<view class="section1 whiteBg divideBottom">
				<evan-form-item label="上期读数" prop="preNum">
					<template v-slot:main>
						<input class="form-input inputColor" type="digit" disabled="true" placeholder-class="form-input-placeholder" v-model="info.preNum"
						 placeholder="请输入上期读数" />
					</template>
					<template v-slot:tip>
						<image class="inpArrow" src="../../static/right_arrow.png" mode="aspectFit"></image>
			...
			<view class="" v-if="hasCommonEle">
				...
				<view class="section2 whiteBg">
					<evan-form-item label="上期读数" prop="commPreNum">
						<template v-slot:main>
							<input class="form-input inputColor" type="digit" disabled="true" placeholder-class="form-input-placeholder" v-model="info.commPreNum"
							 placeholder="请输入上期读数" />
						</template>
						<template v-slot:tip>
							<image class="inpArrow" src="../../static/right_arrow.png" mode="aspectFit"></image>
					...
					.then(res => {
						...
						this.billInfo.items.forEach((item, index) => {
							if (item.itemName == '电费') {
								...
								_this.info.curNum = _this.eleInfo.currentNum;
								...
								_this.info.commCurNum = _this.commEleInfo.currentNum;
