<template>
	<view class="addRoomNum">
		...
		<view class="section1">
			...
			<evan-form class="evanForm" :hide-required-asterisk="hideRequiredAsterisk" ref="form" :model="info">
				...
				<evan-form-item label="户型" prop="houseType" :fontBold="true">
					<template v-slot:main>
						<input class="form-input" disabled="true" placeholder-class="form-input-placeholder" v-model="info.houseType"
						 placeholder="请选择户型" @click="showList()" />
					</template>
					<template v-slot:tip>
		...
		<tip-modal v-if="isShowTipModal1" :oneButton="true" :title="'提示'" :describition="'房号已保存，您可以继续维护卧室信息。'"
		 v-on:emitCancel="hideTipModal"></tip-modal>
		<tip-modal v-if="isShowTipModal" :title="'删除房号'" :describition="'删除房号将删除房号关联的一切信息，是否确认删除?'" v-on:emitCancel="hideTipModal"
		...
		<view v-if="!inEdit" class="saveBtn1" @click="save(false)">保存</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			goRoomDetail(item) {
				console.log(item)
				if(this.returnHouseId || this.houseId){
					...
					uni.navigateTo({
						url: `../roomDetail/roomDetail?isWholeRent=${this.isWholeRent}` + `&roomNo=` + `${item}` + '&houseInfo=' +
							`${JSON.stringify(houseInfo)}`,
					})
				}else{
					this.save(true,item)
